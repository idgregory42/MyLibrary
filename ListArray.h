...
{
    T* item = NULL;
    if (index >= 1 && index <= sz) item = items[index - 1];
    return item;
}
...
{
    if (index < 1 || index > sz + 1) return;
    if (sz == max_size) arrayResize(2*max_size); 
...
{
	return new ListArrayIterator<T>(items, sz);
}
...
void ListArray<T>::set(int index, T* item)
{
    ...
    if (index >= 1 && index <= sz) items[index - 1] = item;  //overwrite contents at that location
}
